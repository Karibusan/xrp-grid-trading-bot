#!/bin/bash
# Script de déploiement personnalisé pour le Synology NAS de Karibusan
...
# Fonction pour exécuter des commandes SSH avec le port personnalisé
function ssh_cmd {
    ssh -p $SYNOLOGY_PORT $SYNOLOGY_USER@$SYNOLOGY_IP "$1"
}

# Fonction pour copier des fichiers avec le port personnalisé
function scp_cmd {
    scp -P $SYNOLOGY_PORT $1 $SYNOLOGY_USER@$SYNOLOGY_IP:$2
}
